      ...
      if (valueTitle && valueStore && value && valueDescription) {
          ...
          if (ValueImage == null) {
              Alert.alert('사진없이 업로드 하시겠습니까?', '', [
                  {
                      text: '예',
                      onPress: () => uploadData(valueTitle,valueStore,value,valueDescription)
    ...
    return (
        <View style={styles.container}>
          <View style={styles.header}>
            <TouchableOpacity style={{flex:2,marginLeft:10}} onPress={()=>navigation.navigate("Home")}><Ionicons name={'backspace'} size={38} color={"#FF480E"}/></TouchableOpacity>
            <View style={{flex:8}}><Text style={styles.headerTitle}>글 작성하기 </Text></View>
            <View style={{flex:2,paddingRight:15}}><Button style={styles.headerButton} onPress={()=>alertText()} title="완료" color={"#FF480E"}></Button></View>
           ...
           <View style={styles.description}>
            ...
            multiline={true}
            />
            <TextInput style={styles.textIputDescription}
            returnKeyType="done"
